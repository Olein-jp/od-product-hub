...
3. SDKが現在の `plugin_version` を付けて `POST /od-product-hub/v1/updates/check` を呼ぶ。Hubは商品・ライセンス・契約を検証してから公開版とクライアント版を比較する。同一版またはクライアント側が新しい場合はZIPを読み込まず、DBへダウンロード権も書き込まず、`success` と `update_available: false` だけを返す。公開版が新しい場合に限り保存ZIPのSHA-256とEd25519署名を毎回検証し、成功した場合だけ5分以内（設定範囲60〜900秒）の一回利用URLを発行する。不正形式またはバージョン省略はHTTP 400とし、ダウンロード権を発行しない。検証結果はキャッシュしないため、ファイル差し替え後に古い成功結果が再利用されることはない。
...
更新確認のAPIログで `release_package_missing` を検出した場合は、秘密ストレージのマウント、権限、対象ファイルの配置を確認し、対象リリースを `wp odph release withdraw <id>` で停止してください。`release_package_integrity_failed` を検出した場合も対象リリースを直ちにwithdrawし、パッケージを隔離してビルド元を調査します。原因解消後は既存レコードやファイルを直接差し替えず、検証済みの新しいバージョンを `wp odph release publish` で公開してください。ストレージの実パス、署名本体、秘密鍵はAPIレスポンスやSite Healthのコピー可能な情報へ出力されません。
