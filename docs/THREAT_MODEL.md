...
| --- | --- | --- | --- |
| Stripe Webhook | 署名偽造、リプレイ、並行重複、処理失敗の取りこぼし、巨大/機密payload | Stripe署名検証、イベントID一意制約、失敗/stale状態の原子的な再claim、5回の試行上限と終端状態、nonce付き手動再開、保存前/表示時マスク | Secret漏洩時はローテーション。終端イベントは原因解消後にStripeから再送し、ログ保持期間とアクセス権を維持する |
