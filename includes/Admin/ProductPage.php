...
final class ProductPage {
	...
	public function render(): void {
		...
		foreach ( $result->items as $p ) {
			...
			printf( '<tr><td>%1$s</td><td><code>%2$s</code></td><td><a href="https://dashboard.stripe.com/products/%3$s" target="_blank" rel="noopener noreferrer"><code>%4$s</code><span class="screen-reader-text">%10$s</span></a></td><td><code>%5$s</code></td><td>%6$s</td><td><a href="%7$s">%11$s</a> | <a href="%8$s">%9$s</a></td></tr>', esc_html( $p->name ), esc_html( $p->slug ), esc_attr( $p->stripe_product_id ), esc_html( $p->stripe_product_id ), esc_html( $p->stripe_price_id ), esc_html( $p->status ), esc_url( $edit_url ), esc_url( $status_url ), esc_html( 'active' === $p->status ? __( 'Deactivate', 'od-product-hub' ) : __( 'Reactivate', 'od-product-hub' ) ), esc_html__( '(opens in a new tab)', 'od-product-hub' ), esc_html__( 'Edit', 'od-product-hub' ) );
		}
		echo '</tbody></table>';
		echo wp_kses_post(
			paginate_links(
